...
from models.pretrained import build_pretrained_model, unfreeze_for_finetuning
...
def run_mobilenetv4(train_df, val_df, test_df, pos_weight):
    ...
    from models.mobilenetv4 import build_mobilenetv4
...
if __name__ == "__main__":
    # Tee all prints to outputs/results/training_log.txt
    # NOTE: using append mode ("a") so this run's log doesn't overwrite the
    # previous run's log (which contains custom_cnn + densenet121 results).
    log_path = os.path.join(config.RESULTS_DIR, "training_log.txt")
    log_file = open(log_path, "a", encoding="utf-8")
    sys.stdout = Tee(sys.__stdout__, log_file)
    print("\n" + "#" * 60)
    print("# RESUMED RUN — resnet50 + efficientnetb4 + medmamba")
    print("#" * 60)
    ...
    # 2. Define runs as data: (name, build_fn, img_size, two_phase, batch_size)
    #    custom_cnn and densenet121 are commented out — they finished in the
    #    previous run and their plots/predictions are already saved in outputs/.
    runs = [
        # ("custom_cnn",
    ...
    # 3. MobileNetV4 (optional — comment out if not installed)
    try:
        results["mobilenetv4"] = run_mobilenetv4(train_df, val_df, test_df, pos_weight)
    except Exception as e:
        print(f"!! mobilenetv4 failed: {e}")

    # 4. Final comparison table (only for models run in THIS session)
